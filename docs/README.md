...
| WikibaseCitation (D4) | extension (`action=citation`, citeproc-php; cite-by-QID parser functions `{{#cite:Q42}}` / `{{#citations:}}` — `docs/decisions/cite-by-qid.md`, issues #24/#25) | `/var/www/wikibase/extensions/WikibaseCitation/` |
...
As of the **Aug 17 2026 v1 deployment**: seeded vocabulary — **31 properties** (incl. 8 ExternalId authority properties with formatter URLs: Wikidata ID, ORCID, VIAF, ISNI, DOI, ISBN-13, OpenAlex Work ID, PubMed ID; plus citation metadata: given/family name, published in, publisher, page(s), volume, issue; plus **image (P32, url datatype — attaches files/media to items, aligned to Wikidata P18 + schema.org, added Aug 19 2026)**) and **13 classes** (quotation content, code snippet, mathematical expression, programming language, person, organization, group of humans, book, scholarly article, website, song, film, video), **80 Pygments language items**, and 5 dogfood entities (person/book/quotation/code/math). The seed's self-verification is green; `Special:AddPerson` / `AddSource` / `AddCollective` import entities from external authorities (Wikidata hub + dblp SPARQL, OpenAlex, Crossref, Open Library, ORCID). P27 is a gap on the instance (deleted during early seeding). As of **Aug 20 2026**: cite-by-QID is implemented (`docs/decisions/cite-by-qid.md`, issues #24 v1 + #25 v2) — `{{#cite:Q42|Q7|style=|output=}}` renders a citation for one or more items from the item graph (usable inside the stock Cite extension's `<ref>`; multi-entity refs in one footnote), `{{#citations:}}` renders the page's deduped source bibliography (accumulated via `{{#cite}}` + embed auto-collect, or explicit `{{#citations:Q42|Q7}}`), citing pages auto-invalidate via templatelinks when an entity is edited, and a `page(s)` qualifier on the `source` statement gives the quote-level locator. The stock **Cite extension must be loaded** (`wfLoadExtension( 'Cite' )` in `LocalSettings.php`) for `<ref>`/`<references/>`; on this stack the dev/CI image does not bundle Cite — CI clones it into the container (see `.github/workflows/ci.yml`). Note: stock Cite on MW 1.46 registers `<references/>`, not the `{{reflist}}` parser function — `{{reflist}}` only works if a `Template:Reflist` exists on the instance.
