...
| Aug 30 batch — webpage parent host-match auto-assign | implemented (awaiting deployment): `Special:AddSource/webpage` now matches the **normalized root-URL host** against website items' URL statements via WDQS (one query; `sparqlUrl` config key, seed-emitted) and **auto-assigns the parent silently** on an exact host match — no [Yes/No] click, deterministic, works even when the site blocks metadata fetching. The site-name inference (banner/hint) remains the fallback when WDQS is unavailable or stale. `SiteRootMatcher` (pure, unit-tested) — see the extensions AGENTS.md |
| Aug 30 batch — OSM place fields | implemented (awaiting deployment): `Special:AddPerson`/`UpdatePerson` place-of-birth/death fields are **OpenStreetMap search comboboxes** (Nominatim, browser-first) storing the canonical `node|way|relation/<id>` on NEW external-id properties (`place of birth (OSM)` / `place of death (OSM)`, formatter URL → openstreetmap.org) — no more manually-created place items. Harvested Wikidata place labels are **auto-matched on Nominatim at harvest-on-pick** with the fetch-match-confirm [Yes/No] banner (the portrait-license pattern); no match → an OSM search hint. A raw place name is rejected on submit (`OsmPlace` server gate). `Template:Person` renders the rows as OSM links. See `docs/decisions/osm-places.md` |
| Aug 30 batch — semantic-first image facts + Add\* upload fixes | implemented (awaiting deployment): the logo/portrait **license/author/license-info now attach to the FILE** — every Add\* upload creates its sitelinked `image`-class item (`ImageItemCreator`, the Special:Upload model) and the File page carries the `== License ==`/`== Attribution ==` blocks; the consumer entity carries only the `image` statement. **Reuse-existing mode** hides the image-fact fields (the reused file already has them) and needs no license. Destination file names are **whitespace→dash normalized** (`European Space Agency` → `File:European-Space-Agency-logo.png`). The validate probe **reads SVG/JPEG-SOF/PNG/GIF/BMP/WebP dimensions** from the capped body (the "probe is capped at 131072 bytes" logo-URL warning is gone). `Special:AddSource`'s **access file survives the content review step** (beforeCreate is idempotent — no more "Select a file to upload."). New **`intergovernmental organization`** class on `Special:AddCollective` (UN/WHO) — see the extensions AGENTS.md |
| Content "Add more" + markup-label page fix + Update\* access collapse | implemented (awaiting deployment): the v1 content pages (`AddQuotation`/`AddCodeSnippet`/`AddMath`) prefill the **label with the parenthetical class marker** (`(quotation)`/`(code snippet)`/`(math snippet)`) and gain an **"Add more"** submit button — creates the item, reopens the page with the provenance carried over (label reset to the prefill, payload empty) for rapid entry from the same source/author. **Harvested titles with HTML markup (`<i>…</i>` from OpenAlex) are now stripped** before they become labels/page titles — the Q1232 bug: the markup made the `Source:` page title invalid, so the classic page was silently skipped (fix + a visible warning when a title is still unusable, + the Update* flow heals missing pages, + `tools/backfill_classic_pages.py` for already-orphaned items). `Special:UpdateSource` **collapses the access section** behind an "I will update the resource access instructions" checkbox (unchecked = access statements untouched). Content-page submits are login-gated. See `docs/decisions/add-more-and-access-collapse.md` |
| Inline LaTeX math — `$…$` (SimpleMathJax, vendored) | implemented + CI green (awaiting deployment): client-side **MathJax 3** typesetting of `$…$` inline and `$$…$$` display math in page prose (plus the `<math>` tag form); self-hosted MathJax 3.2.2 assets (`tools/install-mathjax.sh`, never a CDN); KaTeX rendering of the structured math-snippet items (`Special:AddMath`) unchanged — see `docs/decisions/inline-latex-math.md` |
| Sep 2026 UX batch | implemented + CI green (awaiting deployment): (1) the Add\* **autofill-confirm [Yes]/[No] buttons are delegated** at document level — the OOUI HTMLForm re-creates autoinfuse field layouts from `data-ooui`, orphaning per-node handlers (the dead AddPerson place-of-birth confirm); (2) **Item pages of source items gain "Copy internal citation"** (bookExcerpt has no classic page — the Item page is the cite surface); (3) the Item toolbar's **Copy embed code offers internal `{{#content:Q42}}` vs external iframe**; (4) **AddSource live label preview** (the class-suffix final label shown beside the Title field as you type); (5) **`Special:QuotationsOf` renders each quotation as a blockquote**; (6) **`Special:ChildItemsOf/<Qid>` + `{{#child-items-of:}}`** list a source's child items (book → excerpts, channel → videos); (7) **OSM place labels** — `Person:` pages show the human-readable Nominatim label linked to the OSM map, stored at create/update (`place of birth/death (label)` properties) with `tools/backfill_osm_place_labels.py` for existing items — see `docs/decisions/ux-batch-2026-09.md`, `source-children-listing.md`, `place-labels.md` |
| LanguageBar — automatic languages bar | implemented + CI green (awaiting deployment): the `{{Languages}}` static-translation bar is included by default on every content page (Main/Help/Cheatsheets/HowItWorks/FOSS/Person/Source/Collective/Software) via the `OutputPageBeforeHTML` hook; `Template:Languages` becomes `{{#languagebar:}}` (single markup source); entity/Template/Category/MediaWiki/Special/talk/Forum and the gated RonzzIT:/RonzzInt: namespaces excluded — see `extensions/LanguageBar/AGENTS.md` + `docs/decisions/automatic-languages-bar.md` |
| AddSource round 4 — optional fields, multi-value jurisdiction, international marker, `text` class | implemented (awaiting deployment): **title is the only required field** (authors optional everywhere — a text of unknown authorship is legitimate); the legal texts' **territorial jurisdiction is multi-value** (comma-separated OSM ids + a JSON label map, rendered by `{{#osm-place:jurisdiction}}`); a per-class **"This is an international …"** checkbox replaces the jurisdiction for international texts (new string `international` marker); a new **`text`** catch-all class (Q234460, `Source:` pages transclude `Template:Text`); and the `Special:AddSource/treaty/manual` **`TypeError` 500 fixed** (`createItemAndRedirect`/`onDuplicateCreateSubmit` now return `bool|string`) with a regression E2E — see `docs/decisions/addsource-improvements.md` |
| Add\* form fixes — case-insensitive search, combobox scope, Update prefill, title normalization, NewItem pages | implemented (awaiting deployment): (1) `action=entitysearch` matches labels/aliases **case-insensitively** (`CONVERT(wbx_text USING utf8mb4)`, anywhere in the term — `APACHE` now finds "Apache License 2.0"); (2) the entity-combobox **class scope rides the OOUI widget data** (survives the HTMLForm auto-infusion, which drops DOM attributes) so the operating-system field no longer suggests "The Linux Foundation"; (3) `Special:UpdateSoftware` **prefills the entity comboboxes + programming language** and the page-kind radio; (4) **page titles auto-normalize** (`# < > [ ] { } |` → `-`, label unchanged) so a "C#"/"A|B" label still gets a classic page; (5) **`Special:NewItem` auto-creates a Main-namespace sitelinked page** titled with the label (a `PageSaveComplete` hook gated to the NewItem request) — see `docs/decisions/add-forms-fixes-2026-09.md` |
...
- `extensions/SimpleMathJax/` — vendored third-party inline-LaTeX-math
  extension (`$…$` / `$$…$$` / `<math>` typeset client-side by MathJax 3) —
  see `extensions/SimpleMathJax/VENDORED.md` +
  `docs/decisions/inline-latex-math.md`. The MathJax 3 assets (~24 MB, not
  committed) are installed per environment by `tools/install-mathjax.sh`
  (pinned, sha256-checked; never a CDN at runtime).
- `extensions/LanguageBar/` — automatic languages bar (static-translation
  switcher) on content pages — the `{{Languages}}` bar is injected
  server-side (`OutputPageBeforeHTML`), and `Template:Languages` wraps the
  `{{#languagebar:}}` parser function — see `extensions/LanguageBar/AGENTS.md`
  + `docs/decisions/automatic-languages-bar.md`.
